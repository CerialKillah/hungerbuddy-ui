...
export default function PaymentDetails({items}) {
  const mrpTotal = items.reduce((sum, item) => sum + (Number(item.fullprice) * item.qty) ,0)
  const discount = items.reduce((sum, item) => sum + (Number(item.offerprice) > 0 ? ( Number(item.fullprice) - Number(item.offerprice)) : 0)*item.qty ,0);
  const deliveryFee = 0;
